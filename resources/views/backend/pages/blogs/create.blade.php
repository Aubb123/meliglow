                            ...
                            <div class="row">
                                <div class="col-md-6">   
                                    <div class="form-floating form-floating-outline mb-6">
                                        <input class="form-control" id="title" name="title" type="text" placeholder="Entré le titre du blog ici" value="{{ old('title', '') }}" required>
                                        <label for="title">Titre du blog</label>
                                        @error('title') <div class="text-danger">{{ $message }}</div> @enderror
                                    </div>
                                    <div class="form-floating form-floating-outline mb-6">
                                        <input class="form-control" id="time_read" name="time_read" type="text" placeholder="Ex: 2, 1, 5" value="{{ old('time_read', '') }}" required>
                                ...
                                <div class="col-md-6">   
                                    <div class="form-floating form-floating-outline mb-6">
                                        <select class="form-control" name="is_visible" id="is_visible" required>
                                            ...
                                            <option class="text-secondary" value="0" {{ old('is_visible', "") == '0' ? 'selected' : '' }}>Blog masquer</option>
                                            <option class="text-secondary" value="1" {{ old('is_visible', "") == '1' ? 'selected' : '' }}>Blog démasquer</option>
                                        </select>                                        
                                    ...
                                    <div class="form-floating form-floating-outline mb-6">
                                        <select class="form-control" name="categorie_token" id="categorie_token" required>
                                            ...
                                            @foreach($categories as $categorie)
                                                <option class="text-secondary" value="{{ $categorie->token }}" {{ old('categorie_token', '') == $categorie->token ? 'selected' : '' }}>
                                                    {{ $categorie->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="form-floating form-floating-outline mb-6">
                                    <input class="form-control" id="path_img" name="path_img" type="file" value="{{ old('path_img', '') }}" accept=".png, .jpg" required>
                                ...
                                <div class="form-floating-outline mb-6">
                                    ...
                                    <select class="form-control" name="tags_token[]" id="tags_token" multiple required size="5">
                                        @foreach ($tags as $tag)
                                            <option value="{{ $tag->token }}" {{ in_array($tag->token, old('tags_token', [])) ? 'selected' : '' }}>{{ $tag->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('tags_token')
...
@section('summernote')
    ...
    @endif
    <script>
        var initialContent = "Description du blog";
